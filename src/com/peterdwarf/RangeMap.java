...
public final class RangeMap {
	...
	/**
	 * Put, or update, a mapping between a range of values and an object. The
	 * <code>begin</code> and <code>end</code> values are treated as unsigned
	 * 64-bit integers.
	 */
	...
	/**
	 * Get the object that is mapped to a range containing the argument, or
	 * <code>null</code> if no range is mapped.
	 */
	...
	private class Range {
		...
		boolean overlaps(Range that) {
			return ((ucomp(this.begin, that.begin) <= 0 && ucomp(this.end, that.begin) >= 0) || (ucomp(this.end, that.begin) >= 0 && ucomp(this.end, that.end) <= 0) || (ucomp(
					this.begin, that.end) <= 0 && ucomp(this.end, that.end) >= 0));
		}
